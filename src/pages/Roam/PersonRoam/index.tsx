...
const CzmlCesiumRoam: React.FC = () => {
  ...
  const init = async () => {
    ...
    viewer.current.scene.postProcessStages.fxaa.enabled = true;
    // 自定义着色器
    let customShader: any = new Cesium.CustomShader({
      // 修改灯光模式
    ...
    viewer.current.scene.primitives.add(
      await Cesium.Cesium3DTileset.fromUrl(
        'http://registry.theiavis.com:8099/TheiaWorkflow_2024.04.26-17.38.10/P_ZDQY_DX.json',
      ),
    );
    const tileset = await Cesium.Cesium3DTileset.fromUrl(
      'http://registry.theiavis.com:8099/TheiaWorkflow_2024.04.26-16.12.33_K/P_JXK_JZ.json',
    );
    viewer.current.scene.primitives.add(tileset);
    // viewer.current.zoomTo(tileset);
    tileset.lightingModel = Cesium.LightingModel.UNLIT;
    ...
    models.forEach((item: any) => {
      const entity = viewer.current?.entities.add({
        ...
        model: {
          ...
          color: new Cesium.Color(0.0, 0.2, 0.8, 0.6),
        },
        name: item.name,
      });
        ...
        onUpdate: () => {
          modelList.forEach((item) => {
            ...
            if (item.name === '2f') {
              ...
              item.model.color = new Cesium.Color(0.5, 0.5, 0.5, 1);
            }
            if (item.name === '3f') {
              item.position = Cesium.Cartesian3.fromDegrees(
                113.94708,
    ...
    viewer.current.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(113.94708, 22.552329, 500),
    });
